"""Regression checks for the artifact's RTL integration evidence."""
...
def test_decoder_cluster_is_hierarchical_and_routed() -> None:
    ...
    summary_path = ROOT / "results" / "decoder_cluster_openroad_summary.json"
...
def test_engine_and_cluster_cosim_pass() -> None:
    for rel in (
        "results/encoder_stream_cosim.log",
        "results/decoder_cluster_cosim.log",
    ):
        assert "PASS" in (ROOT / rel).read_text()
    equivalence = (ROOT / "results/encoder_stream_equivalence.csv").read_text()
    assert equivalence.count("PASS") >= 24


def test_physical_summary_does_not_claim_unincluded_activity_power() -> None:
    payload = json.loads((ROOT / "results/decoder_cluster_openroad_summary.json").read_text())
    assert payload["real_trace_vcd_saif"] is False
    assert "No real-trace VCD/SAIF power claim" in payload["note"]
