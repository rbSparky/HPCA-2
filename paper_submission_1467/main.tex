...
Figure~\ref{fig:depth-scaling} connects the layerwise persistence in Supplementary Fig.~S1 to subsystem behavior: valid Reddit improves monotonically from 1.272$\times$ at D8 to 1.324$\times$ at D16, while Arxiv exceeds 1.2$\times$ at D16. Flickr's 0.981$\times$ D16 result bounds the claim---depth helps only when positional persistence coincides with exposed memory traffic.
